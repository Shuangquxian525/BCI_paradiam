...
from fubo_pneumatic_finger import FuboSingleFinger

# trigger = TriggerNeuracle("COM5")
hand_device = FuboSingleFinger(init_params={'port': "COM4"})
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='E:\\pycharm\\MyCode\\Psy_tuto\\手指被动\\finger_touch_test_lastrun.py',
...
def setupWindow(expInfo=None, win=None):
    ...
    if win is None:
        ...
        win = visual.Window(
            size=[600, 500], fullscr=False, screen=0,
...
def run(expInfo, thisExp, win, inputs, globalClock=None, thisSession=None):
    ...
    F1 = visual.ImageStim(
        win=win,
        name='F1', 
        image='图片/F1-removebg-preview.png', mask=None, anchor='center',
    ...
    F2 = visual.ImageStim(
        win=win,
        name='F2', 
        image='图片/F2-removebg-preview.png', mask=None, anchor='center',
        ori=0.0, pos=(0, 0), size=(0.5, 0.5),
        color=[1,1,1], colorSpace='rgb', opacity=None,
        flipHoriz=False, flipVert=False,
    ...
    F3 = visual.ImageStim(
        win=win,
        name='F3', 
        image='图片/F3-removebg-preview.png', mask=None, anchor='center',
        ori=0.0, pos=(0, 0), size=(0.5, 0.5),
        color=[1,1,1], colorSpace='rgb', opacity=None,
        flipHoriz=False, flipVert=False,
    ...
    F4 = visual.ImageStim(
        win=win,
        name='F4', 
        image='图片/F4-removebg-preview.png', mask=None, anchor='center',
        ori=0.0, pos=(0, 0), size=(0.5, 0.5),
        color=[1,1,1], colorSpace='rgb', opacity=None,
        flipHoriz=False, flipVert=False,
    ...
    F5 = visual.ImageStim(
        win=win,
        name='F5', 
        image='图片/F5-removebg-preview.png', mask=None, anchor='center',
        ori=0.0, pos=(0, 0), size=(0.5, 0.5),
        color=[1,1,1], colorSpace='rgb', opacity=None,
        flipHoriz=False, flipVert=False,
    ...
    text_5 = visual.TextStim(win=win, name='text_5',
        ...
        languageStyle='LTR',
        depth=-1.0);
    sound_3 = sound.Sound('音频/ding.wav', secs=-1, stereo=True, hamming=True,
        name='sound_3')
    ...
    trials = data.TrialHandler(nReps=30.0, method='random', 
    ...
    for thisTrial in trials:
        ...
        xtime = 1.6  # 4 + 0.4 * random.random()
        text_4.setText(str(targetlist[F_ind]+1))
        # keep track of which components have finished
        prepComponents = [text_2, text_4, sound_1]
        ...
        while continueRoutine:
            ...
            if text_4.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > text_4.tStartRefresh + xtime-frameTolerance:
                    # keep track of stop time/frame for later
        ...
        Bx = [F1, F2, F3, F4, F5]
        FMs = ('thumbflex', 'indexflex', 'middleflex', 'ringflex', 'littleflex')
        for mm in range(5):
            Bx[mm].setPos([10, 0])
        Bx[targetlist[F_ind]].setPos([0, 0])
            
        hand_device.start(FMs[targetlist[F_ind]])
        # win.callOnFlip(trigger.send_trigger, 0x01+targetlist[F_ind])
        # keep track of which components have finished
        runComponents = [F1, F2, F3, F4, F5]
        ...
        while continueRoutine and routineTimer.getTime() < 2.5:
            ...
            if F1.status == NOT_STARTED and tThisFlip >= 0-frameTolerance:
                # keep track of start time/frame for later
            ...
            if F1.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > F1.tStartRefresh + 2.5-frameTolerance:
                    # keep track of stop time/frame for later
                    F1.tStop = t  # not accounting for scr refresh
                    F1.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'F1.stopped')
                    # update status
                    F1.status = FINISHED
            ...
            if F2.status == NOT_STARTED and tThisFlip >= 0-frameTolerance:
                # keep track of start time/frame for later
                ...
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'F2.started')
                # update status
                F2.status = STARTED
                F2.setAutoDraw(True)
            ...
            if F2.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > F2.tStartRefresh + 2.5-frameTolerance:
                    # keep track of stop time/frame for later
                    F2.tStop = t  # not accounting for scr refresh
                    F2.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'F2.stopped')
                    # update status
                    F2.status = FINISHED
            ...
            if F3.status == NOT_STARTED and tThisFlip >= 0-frameTolerance:
                # keep track of start time/frame for later
                ...
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'F3.started')
                # update status
                F3.status = STARTED
                F3.setAutoDraw(True)
            ...
            if F3.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > F3.tStartRefresh + 2.5-frameTolerance:
                    # keep track of stop time/frame for later
                    F3.tStop = t  # not accounting for scr refresh
                    F3.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'F3.stopped')
                    # update status
                    F3.status = FINISHED
            ...
            if F4.status == NOT_STARTED and tThisFlip >= 0-frameTolerance:
                # keep track of start time/frame for later
                ...
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'F4.started')
                # update status
                F4.status = STARTED
                F4.setAutoDraw(True)
            ...
            if F4.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > F4.tStartRefresh + 2.5-frameTolerance:
                    # keep track of stop time/frame for later
                    F4.tStop = t  # not accounting for scr refresh
                    F4.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'F4.stopped')
                    # update status
                    F4.status = FINISHED
            ...
            if F5.status == NOT_STARTED and tThisFlip >= 0-frameTolerance:
                # keep track of start time/frame for later
                ...
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'F5.started')
                # update status
                F5.status = STARTED
                F5.setAutoDraw(True)
            ...
            if F5.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > F5.tStartRefresh + 2.5-frameTolerance:
                    # keep track of stop time/frame for later
                    F5.tStop = t  # not accounting for scr refresh
                    F5.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'F5.stopped')
                    # update status
                    F5.status = FINISHED
        ...
        sound_3.setSound('音频/ding.wav', hamming=True)
        ...
        # keep track of which components have finished
        restComponents = [rest_fig_2, text_5, sound_3]
        ...
        while continueRoutine:
            ...
            if rest_fig_2.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > rest_fig_2.tStartRefresh + 2.5-frameTolerance:
                    # keep track of stop time/frame for later
            ...
            if sound_3.status == NOT_STARTED and tThisFlip >= 2.5-frameTolerance:
                # keep track of start time/frame for later
